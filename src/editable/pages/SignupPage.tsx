    ...
    <EditableSiteShell>
      <main className="bg-[var(--slot4-dark-bg)] text-white">
        <section className="mx-auto grid min-h-[calc(100vh-10rem)] max-w-[var(--editable-container,1500px)] items-center gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[430px_1fr] lg:px-8">
          <div className="border border-white/10 bg-white p-6 text-[var(--slot4-page-text)] shadow-[0_18px_55px_rgba(0,0,0,0.28)] sm:p-8">
            <h1 className="border-b-2 border-[var(--slot4-accent)] pb-5 text-lg font-black uppercase tracking-[0.04em] text-[var(--slot4-accent)]">Create account</h1>
            <EditableLocalSignupForm />
            <p className="mt-5 text-sm text-[var(--slot4-muted-text)]">Already have an account? <Link href="/login" className="font-black text-[var(--slot4-page-text)] underline-offset-4 hover:underline">Login</Link></p>
          </div>
          <div>
            <p className="text-xs font-black uppercase tracking-[0.24em] text-[var(--slot4-accent-soft)]">Join the desk</p>
            ...
            <div className="mt-9 grid max-w-3xl gap-4 sm:grid-cols-3">
              {[
                ...
                <div key={label as string} className="border border-white/10 bg-white/[0.06] p-5">
                  <Icon className="h-6 w-6 text-[var(--slot4-accent-soft)]" />
