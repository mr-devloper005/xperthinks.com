  ...
  return (
    <Link href={href} className={`group block min-w-0 overflow-hidden ${dc.surface.dark} ${dc.motion.lift}`}>
      <div className="relative min-h-[520px] p-6 sm:p-8 lg:min-h-[620px]">
        <img src={getEditablePostImage(post)} alt={post.title} className="absolute inset-0 h-full w-full object-cover opacity-62 transition duration-500 group-hover:scale-105" />
        <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(0,0,0,0.76),rgba(0,0,0,0.14)_76%)]" />
        <div className="relative z-10 flex h-full min-h-[460px] flex-col justify-end lg:min-h-[560px]">
          <span className="w-fit bg-white/18 px-3 py-1 text-[11px] font-black uppercase tracking-[0.16em] text-white">{label}</span>
          <h3 className="mt-5 max-w-3xl text-4xl font-black leading-[1.04] sm:text-5xl lg:text-6xl">{post.title}</h3>
          <p className="mt-5 max-w-2xl text-sm leading-8 text-white/75 sm:text-base">{getEditableExcerpt(post, 190)}</p>
          <span className={`mt-8 inline-flex w-fit items-center gap-2 bg-white px-5 py-3 text-sm font-black uppercase tracking-[0.08em] ${pal.panelText}`}>
  ...
  return (
    <Link href={href} className={`group ${dc.layout.minRailCard} block overflow-hidden ${dc.surface.card} ${dc.motion.lift}`}>
      ...
      <div className="p-5">
        ...
        <h3 className={`mt-3 line-clamp-3 text-2xl font-black leading-tight ${pal.panelText}`}>{post.title}</h3>
  ...
  return (
    <Link href={href} className={`group block min-w-0 ${dc.surface.soft} p-5 ${dc.motion.lift}`}>
      <div className="flex items-start gap-4">
        <span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-sm ${pal.darkBg} text-xs font-black text-white`}>{index + 1}</span>
        <div className="min-w-0">
          ...
          <h3 className={`mt-2 line-clamp-2 text-xl font-black leading-tight ${pal.panelText}`}>{post.title}</h3>
  ...
  return (
    <Link href={href} className={`group grid min-w-0 gap-5 overflow-hidden ${dc.surface.card} p-4 ${dc.motion.lift} sm:grid-cols-[260px_minmax(0,1fr)]`}>
      ...
      <div className="min-w-0 p-2 sm:py-4 sm:pr-5">
        ...
        <h2 className={`mt-3 line-clamp-3 text-2xl font-black leading-tight ${pal.panelText} sm:text-3xl`}>{post.title}</h2>
        <p className={`mt-4 line-clamp-3 text-sm leading-7 ${pal.softMutedText}`}>{getEditableExcerpt(post, 180)}</p>
        <span className={`mt-5 inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.12em] ${pal.panelText}`}>Open article <ArrowRight className="h-4 w-4" /></span>
      </div>
    </Link>
  )
}
