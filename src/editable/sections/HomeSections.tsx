  ...
  return (
    <div className="mb-8 flex items-end justify-between gap-4 border-b border-black/10 pb-4">
      <div>
        ...
        <span className="mt-3 block h-1 w-7 bg-[var(--slot4-dark-bg)]" />
      </div>
      {action}
    </div>
  )
...
function OverlayStory({ post, href, size = 'large' }: { post: SitePost; href: string; size?: 'large' | 'small' }) {
  return (
    <Link href={href} className={`group relative block overflow-hidden bg-[var(--slot4-dark-bg)] text-white ${size === 'large' ? 'min-h-[470px]' : 'min-h-[320px]'}`}>
  ...
  return (
    <Link href={href} className="group block">
      <div className="relative aspect-[4/3] overflow-hidden bg-[var(--slot4-media-bg)]">
        ...
        <span className="absolute right-4 top-4 flex h-11 w-11 items-center justify-center rounded-full border-4 border-[var(--slot4-accent)] bg-white/85 text-sm font-black">{(index % 9) + 1}</span>
      </div>
...
function SidebarList({ posts, primaryTask, primaryRoute, title = 'Latest posts' }: { posts: SitePost[]; primaryTask: TaskKey; primaryRoute: string; title?: string }) {
  ...
  return (
    <aside className="bg-[var(--slot4-gray)] p-6 lg:p-8">
      <h2 className="border-b-2 border-[var(--slot4-accent)] pb-5 text-lg font-black uppercase tracking-[0.04em] text-[var(--slot4-accent)]">{title}</h2>
        ...
        {posts.slice(0, 4).map((post) => (
          <Link key={post.id || post.slug} href={postHref(primaryTask, post, primaryRoute)} className="group grid grid-cols-[92px_minmax(0,1fr)] gap-4">
            ...
            <div className="min-w-0">
              <p className="text-[11px] font-black uppercase tracking-[0.16em] text-[var(--slot4-accent)]">{getEditableCategory(post)}</p>
              <h3 className="mt-2 line-clamp-3 text-base font-black leading-snug group-hover:text-[var(--slot4-accent)]">{post.title}</h3>
              <p className="mt-2 text-[11px] font-bold uppercase tracking-[0.08em] opacity-45">Latest desk</p>
            </div>
          </Link>
        ))}
      </div>
...
function TinyStory({ post, href }: { post: SitePost; href: string }) {
  return (
    <Link href={href} className="group grid grid-cols-[96px_minmax(0,1fr)] gap-4 border-t border-black/10 pt-5">
      <img src={getEditablePostImage(post)} alt={post.title} className="h-24 w-full object-cover" />
      <div>
        <h3 className="line-clamp-2 text-base font-black leading-tight group-hover:text-[var(--slot4-accent)]">{post.title}</h3>
        <p className="mt-2 text-xs font-bold uppercase tracking-[0.1em] opacity-45">{getEditableCategory(post)}</p>
      </div>
    </Link>
  )
}
    ...
    <section className="bg-[var(--slot4-surface-bg)]">
      <div className="mx-auto max-w-[var(--editable-container,1500px)] px-4 pb-10 pt-8 sm:px-6 lg:px-8">
        <div className="mb-8 grid gap-6 lg:grid-cols-[0.38fr_0.62fr] lg:items-end">
          ...
          <div>
            ...
            <form action="/search" className="mt-5 flex max-w-2xl border border-black/10 bg-white">
              <Search className="ml-4 mt-4 h-5 w-5 opacity-45" />
              ...
              <button className="bg-[var(--slot4-dark-bg)] px-5 text-sm font-black uppercase tracking-[0.08em] text-white">Search</button>
            </form>
          </div>
        </div>
        ...
        <div className="mt-8 grid gap-8 md:grid-cols-2">
          ...
          <div className="border border-black/10 bg-[var(--slot4-warm)] p-7">
            ...
            <p className="mt-4 text-base leading-8 text-[var(--slot4-muted-text)]">Read the latest desk updates, move into task detail pages, and explore connected posts without changing the platform logic underneath.</p>
  ...
  return (
    <section className="bg-[var(--slot4-surface-bg)]">
      <div className="mx-auto max-w-[var(--editable-container,1500px)] px-4 py-12 sm:px-6 lg:px-8">
        <SectionHeader title="Topic lanes" action={<Link href={primaryRoute} className="text-xs font-black uppercase tracking-[0.16em] text-[var(--slot4-accent)]">View all</Link>} />
    ...
    <section className="bg-[var(--slot4-lavender)]">
      <div className="mx-auto grid max-w-[var(--editable-container,1500px)] gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,1fr)_425px] lg:px-8">
            ...
            {list.map((post) => (
              <Link key={post.id || post.slug} href={postHref(primaryTask, post, primaryRoute)} className="group grid gap-6 border-t border-black/10 pt-6 sm:grid-cols-[300px_minmax(0,1fr)]">
                ...
                <div>
                  ...
                  <p className="mt-4 line-clamp-4 text-base leading-8 text-[var(--slot4-muted-text)]">{getEditableExcerpt(post, 230)}</p>
                </div>
              </Link>
            ))}
          </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-[var(--slot4-gray)] p-6 lg:p-8">
            <h2 className="border-b-2 border-[var(--slot4-accent)] pb-5 text-lg font-black uppercase tracking-[0.04em] text-[var(--slot4-accent)]">Top reviews</h2>
            <div className="mt-7 grid gap-5">
  ...
  return (
    <section className="bg-[var(--slot4-surface-bg)] px-4 py-12 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[var(--editable-container,1500px)] border-t border-black/10 pt-10">
        <div className="grid gap-8 bg-[var(--slot4-dark-bg)] p-8 text-white lg:grid-cols-[1fr_auto] lg:items-center lg:p-12">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--slot4-accent-soft)]">{pagesContent.home.cta.badge}</p>
          ...
          <Link href="/contact" className="inline-flex items-center justify-center gap-2 bg-[var(--slot4-accent-fill)] px-7 py-4 text-sm font-black uppercase tracking-[0.08em] text-white">
